...
msg <- "{run,permute}_tool.R --ribo RIBO --rna RNA --samples SAMPLES --out OUT --alpha ALPHA\n"
...
msg_permutate <- "permute_evaluation.R --results FILE1 FILE2 [FILE3, ...] --out OUT"
keys_permute.expected <- c("out", "results")
...
parse_args_permute <- function() {
  ...
  if (!(identical(sort(names(res)), keys_permute.expected))) { stop(msg_permutate, call.=FALSE) }
  for (key in keys_permute.expected) {
    if (key!="results") {if (length(res[[key]])!=1) { stop(msg_permutate, call.=FALSE) }}
  }
  # allow one results
  if (length(res[["results"]])<1) { stop(msg_permutate, call.=FALSE) }
  res
}
